...
어드민 특성상 단순한 CRUD 로직이 많기 때문에 입사 후 적응 중인 JPA 를 하다 가끔 mybatis 로 쿼리를 일일이 작성하는 게 너무 불편하고, 개발 생산성이 떨어진다는 생각을 하게 되었습니다.
...
그간 학습이나, 이직을 위한 과제 정도에서 단편적으로 JPA 를 사용할 때 build.gradle 에 spring-data-jap dependency 만 추가하면 따로 세팅이랄게 없었기에, 가벼운 마음으로 바로 구현을 시작했습니다.

하지만, 늘 인생은 생각대로 흘러기자만 않죠..😭
...
여러 가지 디버깅을 하며 삽질을 하다 발견한 원인은 바로 위 transactionManager 였습니다.
...
### 2. @EnableJpaAuditing
이번에는 이미 많은 분들이 아실 것 같지만, 제가 느끼기엔 실무에 깨알 같은 도움이 되는 내용이라 적어봅니다.
...
실무에서는 여러 상황을 대비하기 위해 대부분의 테이블에 생성자/수정자, 생성일시/수정일시 컬럼을 생성해서 이슈 대응을 하곤 합니다.

그간 mybatis 를 사용하면 매번 해당 컬럼 코드를 복사 붙혀넣기 하기 바빴죠. 

JPA 에서는 해당 어노테이션과 아래와 같이 별도의 공통 entity 클래스를 생성해서 `@EntityListeners(value = {AuditingEntityListener.class})` 를 붙혀주면
...
다른 entity 클래스들은 해당 공통 클래스를 상속받는 것만으로 별도 코드 구현 없이 해당 컬럼들을 관리할 수 있었습니다.
